...
import { Container, Row, Col, Nav, Navbar, NavDropdown, Table, Button, Badge, Form, Modal } from 'react-bootstrap';
import { useDispatch, useSelector } from 'react-redux';
import { useNavigate } from 'react-router-dom';
import UserSidebar from './UserSidebar';
import Loader from './Loader';
import { LoanStatusColor, LoanStatusType } from '../utils/LoanTypes';
import { addCollateralToLoanId, applyLoan, getCollateralByLoanId, getLoans, removeCollateralFromLoanId } from '../features/loan/loanSlice';
import { toast } from 'react-toastify';
import { getCollaterals, resetState } from '../features/collateral/collateralSlice';
import { CollateralType } from '../utils/CollateralTypes';
import { getPromotion } from '../features/promotion/promotionSlice';

const UserDashboard = () => {
  const { email, role } = useSelector(state => state.auth);
  const { isLoading, isError, message, collateral, isSuccess, loans } = useSelector(state => state.loan);
            ...
            <div>
              {
                collateral.length > 0
                      ...
                      return (
                        <Row key={col?.id}>
                          <Col sm="8">
                            ...
                            {`${CollateralType[col?.collateralType]} - ${col?.collateralValue} - ${col?.ownShare}`}
                          </Col>
                          <Col sm="3">
                            <span className='ms-3'>
                              <i className="fa-regular p-1 m-1 fa-trash text-danger fa-md pointer border rounded-2"
                                onClick={() => {
                                  dispatch(removeCollateralFromLoanId({ loanId: loanId, collateralId: col?.id }));
                                  handleClose();
                                }} />
                            </span>
                          </Col>
                  ...
                  : <span className='fs-5 text-info'>No Attached Collaterals Found</span>
              }
            </div>
          </Col>
        </Row>
        <Row>
          <Col>
            ...
            <div>
              ...
              {collaterals?.filter((element) => !collateral.some((c) => c.id === element.id))
                ?.map(col => {
                  return (
                    <Row key={col?.id}>
                      <Col sm="8">
                      ...
                      <Col sm="3">
                        <span className='ms-3'>
                          <i className="fa-regular p-1 m-1 fa-circle-check text-success fa-md pointer border rounded-2"
                            onClick={() => {
                              console.log(loanId, col?.id);
                              dispatch(addCollateralToLoanId({ loanId: loanId, collateralId: col?.id }));
                              handleClose();
                            }} />
                        </span>
                      </Col>
    ...
    <Container fluid className='p-0'>
      <Row className="flex-nowrap">
        ...
        <Col className="col overflow-scroll py-3">
          ...
          <marquee className="bg-dark text-white fs-5 my-3"  direction="left" scrollamount="3" behavior="scroll"> {promotion?.message}</marquee>
          {
            isLoading
              ? <Loader />
              : <Container>
                          ...
                          return (
                            <tr key={loan?.loanId}>
                              ...
                              <td className='text-center'>
                                <Badge bg={LoanStatusColor[loan?.state]}>{LoanStatusType[loan?.state]}</Badge>
                              </td>
                              <td className='text-center'>
                                <i className={loan?.state === 0 ? 'fa-regular fa-files m-1 px-2 pt-3 fa-xl pointer' : 'text-muted fa-regular fa-files m-1 px-2 pt-3 fa-xl '}
                                  onClick={() => {
                                    if (loan?.state === 0) {
                                      dispatch(getCollateralByLoanId(loan?.loanId));
                                      handleShow(true);
                              ...
                              <td className='text-center'>
                                <i className={
                                  ...
                                  onClick={() => {
                                    loan?.state === 0 &&
                                      nav(`/UserDashboard/editLoan/${loan?.loanId}`)
                                  }} />
                                <i className={
                                  loan?.state === 0
